...
- Record waiting separately from goal status and scheduler status. Follow the host's repeated-blocker rule without inventing progress; report whether an independently authorized schedule will recheck until the window ends. Preserve message identifiers and prior actions to prevent duplicate notifications.
